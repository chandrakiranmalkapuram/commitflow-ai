...
import { GitBranch, CheckCircle2, Unplug } from 'lucide-react';
import type { GithubAccount } from '../github.types';

export function GithubConnectionCard({ account, isLoading }: { account?: GithubAccount | null, isLoading: boolean }) {
  const handleConnect = () => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          <div className="animate-pulse flex items-center gap-4 border border-gray-100 rounded-lg p-4 bg-gray-50">
          ...
          <div className="flex items-center justify-between border border-green-100 rounded-lg p-4 bg-green-50/30">
            <div className="flex items-center gap-4">
              {account.avatarUrl ? (
                <img src={account.avatarUrl} alt={account.username} className="h-12 w-12 rounded-full border border-gray-200" />
              ) : (
            ...
            <button
              className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors border border-gray-200 bg-white hover:bg-gray-100 text-gray-600 h-9 px-4"
              disabled
              title="Disconnecting is not supported in this preview"
            >
          ...
          <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-lg p-8 bg-gray-50/50">
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors bg-gray-900 text-white hover:bg-gray-800 h-10 px-6 shadow-sm"
            >
